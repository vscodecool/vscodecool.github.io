...
这一节课，我们将学习和文件导航的相关技巧，学习了这些技巧后，我们就可以在多个已打开文件之间做快速切换，以及我们可以在单个文件内的多个编辑点之间做快速切换。

**多个文件之间**

在项目开发过程中，我们经常会打开多个文件，随着已打开文件的不断增多，我们就需要一种方法能够在这些文件之间做快速切换。

在VS Code中，做文件切换最方便的方式是使用快捷键`Ctrl+Tab`，它的具体使用方法是：1，按住`Ctrl+Tab`；2，不松`Ctrl`，依靠单击`Tab`选择文件；3，释放`Ctrl`打开被选文件。

**单个文件内部**

我们经常会在同一个文件的多个地方做编辑，每一次编辑都会产生一个编辑点，英文叫`Edit Location`，我们常常需要在这些编辑点之间做快速切换，特别是当文件的内容很长时，编辑点的快速切换就显得更加重要。

在VS Code中，做编辑点切换，我们可以使用快捷键`Ctrl+-`(`-`是键盘上的键盘上的减号键)
