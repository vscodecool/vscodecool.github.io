工欲善其事，必先利其器！你好，我是[阿汤](https://github.com/tsq)，欢迎来到课程《VS Code入门教程》！在这门课程中，我将与你分享VS Code这款代码编辑器的使用方法，希望能够为你的学习或者工作带来一定的帮助。
...
这个选项的名字叫做**Preserve Case**，翻译成中文就是“保留大小写”。它的具体作用是：当我们做替换操作时，匹配结果中如果包含了一部分大小写不一致的匹配项，使用Preserve Case可以让替换值也跟随原本的大小写规则。
...
这个快捷键非常简单，只需要按一下键盘上的**点** <kbd>.</kbd> 键即可。（需要注意的是，你需要先登陆Github，才能使用这个快捷键。）

顺便说一下，这个基于浏览器的VS Code其实背后用的是Github的**Codespaces**，它可以为开发者提供基于云端的IDE。如果你对此感兴趣，可以前往[https://github.dev](https://github.dev)查看详情。
...
第二种是在项目根目录下创建一个文件，当我们在项目列表的空白区域双击时，就会出现文件名输入框，输入完文件名，文件就被创建完成。如果结合第52课[文件使用技巧8: 快速创建文件或文件夹](#52-文件使用技巧8-快速创建文件或文件夹)学到的知识，我们还可以在快速创建文件的同时，也将文件所属的文件夹也一并创建。

#### 75 文件使用技巧31: 配置新建文件的默认语言

当我们在VS Code中新建一个临时文件的时候，默认这个临时文件采用的文件类型是`Plain Text`，类似txt文件类型，而当我们发现自己每次新建的临时文件并不希望是`Plain Text`，而是比如：json、markdown、javascript等，相较于`Plain Text`这些文件类型可以让输入的内容有语法高亮、智能提示等功能。面对这种情况，为了避免每次都手动的将`Plain Text`调整为期待的文件类型，我们可以通过配置来更改新建临时文件的文件类型。

我们可以通过输入关键词“files default language”在Settings中找到这个配置项，之后在输入框中填入想要的文件类型即可，比如填写`markdown`后，新建临时文件的默认类型就改为markdown了。

另外这个输入框还支持填入一个特殊的值：`${activeEditorLanguage}`，它的意思是新建临时文件的类型**动态的来自于当前被激活的文件**。

#### 76 文件使用技巧32: 打开历史项目快捷键

在项目开发的过程中，我们有时候想打开之前的某个项目，这一节课我们就来学习一下在VS Code中如何通过快捷键来快速打开某个历史项目。

这个快捷键是： <kbd>Ctrl</kbd>+<kbd>R</kbd>.
...
为了查看两个文件在内容上的差异，我们在前面的[50 文件使用技巧6: 文件比较](#50-文件使用技巧6-文件比较)已经学过一种方法，这一节课我们来学习另外两种方法。

**方法二**和上一节课的`查看任意两个commit之间差异`的方式一致：
...
其中`demo.c:4:24`，就在告诉我们，发生错误的文件名叫"demo.c"，错误点在它的第4行第24列。大部分的编程语言或框架在出现错误时，都会以类似的形式来告诉我们是哪里出了问题。

有了，文件名、行、列这三个信息，下面我们就来分两个场景看一下如何快速的定位出错点：

**场景一：错误文件已在VS Code中打开**

因为错误文件已经被打开了，所以只需要跳转到指定的行与列就行，这时，我们可以使用快捷键<kbd>Ctrl</kbd>+<kbd>G</kbd>，接着在弹出框中输入`line:column`，最后回车。

**场景二：错误文件尚未被打开**

使用`Quick Open`快捷键，在macOS上是<kbd>Cmd</kbd>+<kbd>P</kbd>；在windows上是<kbd>Ctrl</kbd>+<kbd>P</kbd>，接着在弹出框中输入`name:line:column`（P.S 这里的`name`不需要完整书写，只需要保证搜索结果列表的第一项就是我们要找的文件即可），最后回车。
...
将鼠标悬浮在出错文件的上方，接着，如果是macOS，按住<kbd>Cmd</kbd>，如果是windows，按住<kbd>Ctrl</kbd>，这时候文件名的下方应该会出现下划线，最后只需要做一次单击，VS Code就会自动定位到出错文件以及具体的行和列。
...
在菜单项“File > New File...”或者使用命令`Create: New File...`，就可以在创建文件的时候先选择一种文件类型，默认只有纯文本类型和Jupiter Notebook类型，当我们安装了一些第三方扩展之后，这里的文件选项也会相应的增多。
...
在项目开发的过程中，随着打开文件的增多，Explorer区域的垂直方向的空间会被占用的越来越多，整个项目目录会看起来非常的杂乱，这就会阻碍我们快速找到某个想要打开的文件。这时候，我们可以使用Explorer顶部工具栏的最右侧的`Collapse`功能，点击就可以一次性折叠所有的文件夹。

#### 94 文件使用技巧50: 快速查看修改点

修改了某个文件之后，在按保存之前，有时候我们会突然想了解一下刚刚到底对文件做了哪些变更。面对这种情况，我们就可以使用命令`Compare Active File with Saved`，使用它，VS Code就会打开一个差异视图，差异视图的左边是上一次的保存版本，右边是当前未保存的版本。通过这个差异视图，就可以很方便的查看所有的修改点，以及快速取消某一处修改点。
...
从**1.85**版本开始，VS Code开始支持粘贴来自从文件系统中的复制。这个功能非常有用，将大大方便我们把文件或者文件夹直接添加到项目当中。
...
#### 97 文件使用技巧53: 使用Tree模式查看搜索结果

搜索结果默认采用的`List`模式，当我们更加关注于结果是来自于哪个文件夹的时候，可以点击顶部的`Tree`按钮，这样结果将会以树状的形式显示，方便我们看清楚所属的文件夹以及折叠某个文件夹。

#### 98 文件使用技巧54: 文件夹限定搜索

搜索结果在采用的`Tree`模式下，我们可以在某个目录上鼠标右键，然后选择“Restrict Search to Folder”来将结果缩小到这个目录下面。当搜索结果很多时，这可以帮助我们有效的缩小搜索范围。

### 第四章 自定义VS Code

这一章节，我们将学习如何配置 VS Code，使其更符合我们每个人的个性化需求。
...
- [105 隐藏工具栏图标](#105-隐藏工具栏图标)
- [106 Windows复制路径设置成正斜杠](#106-windows复制路径设置成正斜杠)
- [107 快速撤销对文件的误操作](#107-快速撤销对文件的误操作)
- [108 配置撤销警告弹窗](#108-配置撤销警告弹窗)
- [109 配置文件列表的粘性滚动](#109-配置文件列表的粘性滚动)
- [110 同类型文件嵌套显示](#110-同类型文件嵌套显示)
- [111 配置WorkspaceTrust](#111-配置workspace-trust)
- [112 启用垂直标尺功能](#112-启用垂直标尺功能)
- [113 禁止自动更新](#113-禁止自动更新)
- [114 关闭链接弹窗](#114-关闭链接弹窗)
- [115 使用内置浏览器](#115-使用内置浏览器)
- [116 减少动画效果](#116-减少动画效果)
- [117 忽略大文件目录](#117-忽略大文件目录)
- [118 让光标变得更加丝滑](#118-让光标变得更加丝滑)
- [119 自定义窗口标题](#119-自定义窗口标题)
- [120 开启通知勿扰模式](#120-开启通知勿扰模式)
- [121 取消代码置顶](#121-取消代码置顶)
- [122 退出前的二次确认](#122-退出前的二次确认)
- [123 配置界面缩放](#123-配置界面缩放)
- [124 share功能](#124-share功能)
- [125 让侧边栏变得更加简洁](#125-让侧边栏变得更加简洁)
- [126 隐藏minimap](#126-隐藏minimap)
...
- [136 启用连字](#136-启用连字)
- [137 向下打开一个Editor](#137-向下打开一个editor)
- [138 Terminal显示空间最大化](#138-terminal显示空间最大化)
- [139 更改标签页滚动条尺寸](#139-更改标签页滚动条尺寸)
- [140 配置项恢复默认值](#140-配置项恢复默认值)
- [141 配置缩进指引线](#141-配置缩进指引线)
- [142 将标签页移动到新窗口](#142-将标签页移动到新窗口)
- [143 找回误删文件](#143-找回误删文件)

#### 99 更改显示语言

VS Code默认使用英文作为显示语言。我们可以通过以下步骤来修改成其它语言，以中文为例：

1. 进入扩展，输入`chinese`。
2. 我们选择第一个“Chinese (Simplified) (简体中文) Language Pack for Visual Studio Code”并点击安装。
3. VS Code会弹出通知提示是否要重启。点击重启后即可生效。

一般而言，一个语音包扩展，名称都包含`Language Pack`，例如韩语，就是“Korean Language Pack for Visual Studio Code”。

当安装了多个语言包之后，下面我们再来看一下如何对其进行切换。

1. 点击“Command Palette”。
...
Sidebar的工具栏中有很多功能图标，如果有些有些功能我们不常用，是可以将其隐藏掉的，这样可以让工具栏变得更加简洁。隐藏的具体方法是：将鼠标放到图标之上，鼠标右键，选择`Hide xxx`。

隐藏之后，如果想再次显示，只需要将鼠标放到任何一个已有的图标之上，鼠标右键，选择`xxx`(可以看到，它的前面没有打勾)。或者点击最后一项`Reset Menu`，它会将工具栏恢复到默认状态。

#### 106 Windows复制路径设置成正斜杠

在Windows上鼠标右键某个文件，然后选择`Copy Relative Path`可以获取某个文件它在项目中的相对路径。这个路径，因为是Windows操作系统，路径分隔符默认采用的是反斜杠。
...
有时候我们会不小心对文件做了一些误操作，这时候在VS Code中是很容易对其进行撤销的，只需要按下：
...
在做撤销操作的时候，默认 VS Code 会弹出确认窗口，有时候可能会觉得这周行为很麻烦，这时候，我们可以通过以下配置项来改变这种行为。

在 Settings 中找到配置项`Confirm Undo`，它有 3 个值:

- `verbose`: 所有撤销操作都弹窗
- `default`: 默认行为
- `light`: 所有撤销操作都不弹窗(创建文件的撤销操作除外)

#### 109 配置文件列表的粘性滚动

在`Explorer`项目文件列表区域，当某个文件夹下面的文件或文件夹很多时，在向下滚动时，VS Code 默认会将那个文件夹置顶显示，如何你不喜欢这种行为，是可以通过以下方式来修改的。

在 Settings 中找到配置项`Workbench Tree: Enable Sticky Scroll`，将打勾取消即可。

#### 110 同类型文件嵌套显示

对于关联性很强的几个文件，例如 app.ts 和 app.js，style.scss 和 style.css，在一些其它的编辑器当中，当遇到这种情况，它们会将这些同类型的文件做嵌套显示，以此来让界面更加简洁。

VS Code 其实也支持这种行为，只是默认处于关闭状态，为了开启这个功能，我们只需要在设置项中找到`File Nesting: Enabled`，将它打勾就行。在它的下方我们也可以看到`File Nesting: Patterns`配置项，这个地方就是用来配置哪些文件之间可以嵌套显示。我们也可以通过点击按钮，来新增一个规则，比如：

- `*.scss` -> `${capture}.css`
- `*.cshtml` -> `${capture}.cshtml.cs`

#### 111 配置Workspace Trust

`Workspace Trust` 是 VS Code 在 2021 年引入的一项安全特性,它主要目的是保护用户免受不信任代码的潜在危害，因为VS Code的一些功能，像是Task是可以在VS Code打开项目的时候会自动运行代码程序，这就会造成潜在的安全风险。因此，如今每当我们用VS Code打开一个文件夹的时候，VS Code都会弹出提示框，询问我们是否要信任这个目录。

这个功能的出发点是好的，但是对于我们大部分普通用户而言，却是增添了麻烦。所以我们来看一下，如何禁用`Workspace Trust`。

只需要找到配置项：`security.workspace.trust.enabled`，将它前面的勾给去掉就可以了。

#### 112 启用垂直标尺功能

VSCode有垂直标尺(Vertical rulers)功能，开启后会在编辑器界面出现几条垂直的标尺。这些标尺只是视觉辅助工具，不会影响代码的实际执行，但能帮助开发者保持良好的编码习惯和代码风格。

具体的使用场景有：

1. 代码长度限制：在团队开发中，通常会规定每行代码的最大长度，比如常见的是 80 或 120 个字符。设置垂直标尺可以让开发者在编码时直观地看到这个界限。

2. 注释对齐和格式化，如SQL语句的注释

配置垂直标尺的具体方法是，在配置JSON中，添加如下配置项：

```
"editor.rulers": [80, 120]
```

这个配置项是个数组，数组的每一项是个整数，代表在第几列出现标尺。

#### 113 禁止自动更新

默认，VS Code自身以及安装的所有扩展，当有新版本的时候，都是会自动更新的。但如果你不想自动更新，我们来看一下如何将它们关闭。

为了取消VS Code自身的自动更新，只需要添加配置项：

```
"update.mode": "none"
```

为了取消扩展的自动更新，只需要添加配置项：
...
在VS Code中，如果一段文本是一个URL链接，那么当把鼠标放上去的时候，就出现一个可点击的按钮，点击就可以在浏览器中打开。
不过，默认VS Code会每次弹窗询问是否需要真的打开它，这就显得有点不方便了。我们可以通过如下配置，将弹窗给去掉。

1. 打开“Command Palette”，找到命令`Manage Trusted Domains`并执行
2. 在打开的JSON文件中，追加`["*"]`

#### 115 使用内置浏览器

当我们在VS Code中选择打开一个文件夹或是保存一个临时文件的时候，都会调用系统默认的文件浏览器。除了这种打开方式，我们可以修改以下配置项来
替换成VS Code内置的文件浏览器，这样，我们不用离开VS Code，就能完成所有的操作。

1. 在Settings中搜索`simple dialog`
2. 将这个配置项给勾选


#### 116 减少动画效果

VS Code的整个UI界面，很多地方默认都会有一定的动画效果。如果你不喜欢，可以通过如下操作，将动画效果给去掉：

1. 打开Settings，搜索`reduce motion`
2. 将它的值改为`on`

#### 117 忽略大文件目录

当使用VS Code打开一个项目文件夹的时候，VS Code其实会在后台分析我们项目里面的所有源文件。此时，如果一个文件夹它里面的文件个数过多，那么可能会导致
VS Code运行缓慢。这时候，我们可以通过以下配置，将特定的文件夹给忽略，这样VS Code就不会对这个文件夹进行分析了。

1. 打开Settings，搜索`files exclude`
2. 点击新增按钮，填入文件夹的名称


#### 118 让光标变得更加丝滑

可以通过修改以下两个配置项，让VS Code的光标在移动的时候，看上去非常的丝滑。
...
VS Code的右下角，会经常跳出一些通知弹窗，如果你觉得它们对你产生了干扰，可以通过以下方式开启通知勿扰模式。

1. 点击界面最右下角的小铃铛
2. 点击右侧第二个图标并点击`Enable Do Not Disturb Mode`

#### 121 取消代码置顶

默认，当我们向下滚动代码的时候，VS Code会对一部分代码置顶显示，如果你不喜欢这种行为，我们可以通过修改以下配置项来取消：

- 找到配置项`Editor Sticky Scroll: Enabled`
- 去掉勾选

如果你喜欢代码置顶的效果，我们可以通过修改配置项`Editor Sticky Scroll: Max Line Count`来调整最多置顶显示几行代码。

#### 122 退出前的二次确认

通过修改配置项`Window: Confirm before close`，我就可以控制是否在退出VS Code时候做二次确认，以防我们误退出VS Code。

这个配置项有个值，叫做`keyboardOnly`，表示只有当使用快捷键(Cmd/Ctrl+Q)退出VS Code的时候，才会弹窗做二次确认。

#### 123 配置界面缩放
如果你觉得VS Code默认的界面偏小或者偏大，我们通过修改配置项`window zoomLevel`来做相应的调整。默认值为0，每次增加/减少都会将缩放级别更改20%。
最大值是8，最小值是-8。

除了修改这个配置项，我觉得更方便的是通过快捷键，mac是Cmd键，windows是Ctrl键，之后再按键盘上的加减键，就能快速缩放界面了。


#### 124 share功能

如果当前项目是托管在Github上面，我们可以通过以下方式，快速获取到这个项目的`https://vscode.dev`地址(由Github提供的网页版VSCode)：菜单项**File->Share->Copy vscode.dev Link**

我们还可以在某个文件的某一行，鼠标右键，选择**Share->Copy vscode.dev Link**，这样生成的链接，就可以直接打开这个文件并跳转到指定的行号。



#### 125 让侧边栏变得更加简洁

VSCode的侧边栏区域通常由多个模块组成，比如，Explorer中默认就会有：Open Editors, Outline, Timeline等模块，如果你平时不太用到这些模块，这时候我们可以选择将它们给隐藏，隐藏某个模块的方法是：点击右上方的三个点，点击将模块名前方的勾去掉(再次点击，模块又会被重新显示)。

#### 126 隐藏minimap

默认当我们打开某个文件的时候，编辑器的右侧会显示这个文件的minimap。对于一个内容较多的文件，minimap还是挺有帮助的，它不仅可以让我们快速移动代码到指定的区域，而且还可以帮助我们对文件的内容，在整体上做一个大致的了解。

但如果你觉得minimap对你而言没有什么太大的用处，并且还觉得它占用了一部分编辑器的空间，这时候我们可以通过以下方式隐藏minimap：

- 打开Settings，输入关键词`minimap`
- 找到配置项`Editor> Minimap: Enabled`，将它的勾选去掉

#### 127 调整panel的占用空间

VSCode底部的Panel，它的占用空间是可以进行调整的，可以调整成占用更少的空间，也可以调整成占用更多的空间。

为了调整，我需要使用一个命令，打开"Command Palette"输入关键词“set panel”，就可以看到四个选项，分别是right, left, center, justify。
当有两个sidebar时（主侧边栏和第二侧边栏），上面的四个配置，它们的显示效果会变得更加明显，center会让panel占用更少的空间，夹杂两个sidebar之间；而justify会占据更多的空间，适合希望panel能显示更多内容的场景。


#### 128 开启文本自动换行

当一行文本的内容过长时，vscode默认并不会对它进行换行，而是会出现水平滚动条，需要我们手动的向右滑动来查看后面的内容。

对于长文本，除了这种默认的方式，有时候我们可能希望vscode能够对这一行做自动换行。

为了配置vscode能够自动换行，在Settings中搜索配置项"word wrap"，将它的值改为`on`就可以了。

#### 129 开启自动保存

VSCode默认是需要手动触发保存的。配置项“Auto Save”，有下面几个选项，可以配置自动保存：
...
如果你经常使用minimap，那么我可以将垂直方向的滚动条给隐藏，这样可以节省一部分空间。
隐藏的具体办法是：

- 找到配置项“Editor › Scrollbar: Vertical”，选择“hidden”
...
点击编辑器右上角的按钮，默认，新建的Editor是出现在右侧，而如果你的显示器是竖屏，垂直空间比较多，这时候，可能就希望向下打开一个新的Editor了。为了实现这个效果，我们按住以下按键，再点击分离图标：
...
有时候我们想让Terminal拥有更多的显示空间，这时候可以使用以下两个命令：
...
更改标签页滚动条的尺寸，可以让拖动变得更加容易。

具体设置路径为：`Workbench Editor: Title Scrollbar Sizing`


#### 140 配置项恢复默认值

在设置页面使用`@modified`可以过滤出做过变更的配置项，然后鼠标移至配置项左侧，就可以看到齿轮图标，
点击里面的'Reset Setting'就可以恢复该项为默认值。

除了通过UI界面，也可以直接打开配置文件，删除相应配置项的json键值对。

另外，如果想以JSON格式的显示vscode所有配置项的默认值，可以使用命令`Open Default JSON`

#### 141 配置缩进指引线

编辑器默认会显示缩进指引（竖线），可以帮助你快速看到匹配的缩进层级。如果你想要禁用缩进指引，
可以在用户或工作区设置中将 `editor.guides.indentation` 设置为 `false`。

#### 142 将标签页移动到新窗口

如果你正在专注于某个源代码文件，并且有另外一个显示器，我们可以通过"Move Editor into New Windows"命令，将当前标签页分离并移动到一个独立的新窗口中，这样就可以使用最大的空间来显示代码。


#### 143 找回误删文件

当我们不小心删除了某个重要文件后，只要这个文件在VS Code中曾经被保存过，就可以通过以下简单步骤轻松找回：

1. 打开命令面板（Windows: Ctrl+Shift+P，Mac: Command+Shift+P）
2. 输入并选择"Local History: Find Entry to Restore"命令
3. 在Timeline视图中从Quick Pick选择你想恢复的文件
